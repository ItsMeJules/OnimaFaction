...
public class LivesGiveArgument extends BasicCommandArgument {

	public LivesGiveArgument() {
		super("give", OnimaPerm.ONIMAFACTION_LIVES_GIVE_ARGUMENT);
		
		usage = new JSONMessage("§7/lives " + name + " <player> <amount>", "§d§oPermet de donner des vies.");
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		OfflineFPlayer.getPlayer(uuid, offlineFPlayer -> {
			...
			sender.sendMessage("§dVous §7avez give §d" + amount + life + " à §d" + Methods.getName(offlineFPlayer.getOfflineApiPlayer(), true) + "§7.");
				
			if (offlineFPlayer.getOfflineApiPlayer().isOnline())
				((APIPlayer) offlineFPlayer.getOfflineApiPlayer()).sendMessage("§d" + Methods.getRealName(sender) + " §7vous a give §d" + amount + life + "§7.");
