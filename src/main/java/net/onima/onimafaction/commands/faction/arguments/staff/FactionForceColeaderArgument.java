...
public class FactionForceColeaderArgument extends FactionArgument {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (fPlayer.getRole() == Role.COLEADER) {
			sender.sendMessage("§c" + offline.getName() + " est déjà co-leader de sa faction !");
			return false;
		}
		
		fPlayer.setRole(Role.COLEADER);
		
		sender.sendMessage("§d§oVous §7avez défini le rôle de §d§o" + offline.getName() + " §7sur co-leader.");
		faction.broadcast("§d§o" + sender.getName() + " §7a défini §d§o" + offline.getName() + " §7en tant que co-leader de la faction.");
		return true;
	}
	...
	public List<String> onTabComplete(CommandSender sender, Command cmd, String label, String[] args) {
		...
		return Arrays.stream(Bukkit.getOfflinePlayers()).filter(offline -> OfflineFPlayer.getByOfflinePlayer(offline).getRole() != Role.COLEADER).map(OfflinePlayer::getName).filter(name -> StringUtil.startsWithIgnoreCase(name, args[1])).collect(Collectors.toList());
	}
